...
**`getDocumentContent(userId: UUID, documentId: UUID): DocumentContent`**
- **Authorization**: User must own the document  
...
- **Returns**: List of document metadata (no content), ordered by created_at DESC
- **Note**: Only returns metadata for efficient list rendering in UI
