...
**Code Duplication**: Three custom dialog components (DocumentInfoDialog, ShareModal, ImportFileDialog) each independently implement:
...
- DocumentInfoDialog missing ESC key dismissal
- ShareModal and ImportFileDialog have ESC support
- All three manually handle backdrop clicks differently
- No focus trapping (keyboard focus can escape to elements behind dialog)

**Dependency Bloat**:
- bits-ui and svelte-sonner add unnecessary external dependencies
- UI library wraps basic functionality we can implement directly in Svelte
- Current ui/ components are thin wrappers around bits-ui primitives
- Redundant abstraction layer that obscures behavior and increases bundle size

**Z-Index Fragmentation**:
- Hardcoded z-40 for backdrops, z-50 for content
- No strategy for nested overlays or layering multiple widgets
- Risk of conflicts with future components

**Maintenance Burden**:
- Changes to dialog behavior require updating three+ components
- Easy to introduce inconsistencies when adding features
- Difficult to ensure accessibility across all widgets

### Design Goals

1. **Zero External UI Dependencies**: Build widgets as pure Svelte components without bits-ui or svelte-sonner
2. **Single Source of Truth**: One base component per widget type that all instances derive from
3. **Eliminate Duplication**: Shared structure, styling, and behavior extracted to base components
4. **Consistent Dismissal**: All widgets support ESC, backdrop click, and close button by default
5. **Accessibility First**: Implement proper ARIA attributes, focus management, and keyboard navigation natively
6. **Flexible Composition**: Base components support customization through Svelte 5 snippets
7. **Maintainable**: Changes propagate automatically through composition
8. **Theme Integrated**: All widgets follow WIDGET_THEME_UNIFICATION.md standards
9. **Reduced Bundle Size**: Eliminate unnecessary dependencies and abstraction layers
